...
### 이번 작업 요약
- 사이드바의 `배포 메모` 섹션을 제거했다.
- 관련 마크업은 `frontend/src/components/Layout/Shell.tsx` 에서 삭제했다.
- 함께 남던 `sidebar-note-section` 전용 스타일도 `frontend/src/index.css` 에서 정리했다.
...
### 이번 작업 요약
- 사이드바에서 `카테고리 구성` 섹션을 제거했다.
- `법령 구성` 섹션은 항상 펼쳐진 상태로 유지하고 `Hide/Show` 버튼도 제거했다.
- 법령 구성과 카테고리 구성이 겹쳐 보이던 문제를 단순 구조로 정리했다.

### 검증
- `frontend/`에서 `npm run build` 통과
- 내부 프론트 재배포 후 `http://127.0.0.1:28088/chat/api/health` 정상
